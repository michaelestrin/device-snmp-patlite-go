...
WORKDIR $GOPATH/src/github.com/edgexfoundry/device-snmp-patlite-go
...
COPY --from=builder /go/src/github.com/edgexfoundry/device-snmp-patlite-go/cmd /

ENTRYPOINT ["/device-snmp-patlite-go","--registry","--profile=docker","--confdir=/res"]
